...
* **`FORMAT 'format'`** 按指定的格式对日期时间进行格式化，返回字符串。下面说明一下格式中各个字母的意义：
    ...
    + **`Q`**或**`q`** 表示季度，1 位返回`3`，2 位返回`03`，3 位返回`Q3`，4 位返回`3rd quarter`。中文环境下返回中文，如`3季`、`第3季度`。
	...
	+ **`S`** 表示毫秒，一般为3位
	+ **`e`**或**`E`** 表示星期，1位返回`1`，2位返回`01`，3位返回`Mon`，4 位返回`Monday`。中文环境下返回中文，如`星期一`。
...
* **`GET QUARTER`** 返回年份，结果为`2019`。
* **`GET MONTH`** 返回月份，结果为`4`。
* **`GET DAY`** 返回日期，结果为`15`。
* **`GET HOUR`** 返回小时，结果为`13`。
* **`GET MINUTE`** 返回分钟，结果为`27`。
* **`GET SECOND`** 返回秒，结果为 `5`。
* **`GET MILLI`** 返回毫秒，秒后 3 位。
* **`GET MICRO`** 返回微秒，秒后 6 位。
* **`GET NANO`** 返回纳秒，秒后 9 位。
* **`GET WEEK`** 返回星期，结果为`1`。星期二返回`2`，星期日返回`7`。
* **`GET WEEK OF YEAR`** 返回日期时间所在周是一年内的第几周，参数为星期简称或全称或数字`1~7`，表示星期几为一周的开始。如`@NOW GET WEEK OF YEAR 'MONDAY'`，不传参数默认“周一”。

以下操作的返回值为字符串：

* **`GET QUARTER NAME`** 返回三位的星期名称字符串，同`FORMAT 'eee'`。
* **`GET MONTH NAME`** 返回三位的星期名称字符串，同`FORMAT 'eee'`。
* **`GET WEEK NAME`** 返回三位的星期名称字符串，同`FORMAT 'eee'`。
* **`GET FULL QUARTER NAME`** 返回三位的星期名称字符串，同`FORMAT 'eee'`。
* **`GET FULL MONTH NAME`** 返回三位的星期名称字符串，同`FORMAT 'eee'`。
* **`GET FULL WEEK NAME`** 返回三位的星期名称字符串，同`FORMAT 'eee'`。
