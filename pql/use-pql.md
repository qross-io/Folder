...
### 运行环境
因为PQL是一种脚本语言，所以其运行环境非常宽松。PQL可以在Java和Scala中像运行SQL一样调用（需要引入依赖），也可以通过Shell命令接运行PQL文件，还可以在[Keeper调度工具](/doc/keeper/overview)中直接运行。PQL还有两个扩展应用，[OneApi统一接口](/doc/oneapi/overview)和[Voyager模板引擎](/doc/voyager/overview)，都基于Spring Boot环境运行。另外，DataHub开发框架作为PQL的底层，也可以运行PQL过程。

### 在线编辑和运行PQL
如果你不会Java或Scala，可以在本机或服务器上部署 **Qross Master** 项目，在线编辑和运行PQL。Master项目部署和运行请参阅[Master数据管理平台](/doc/master/overview)，也可先[线上体验](http://www.qross.cn:8080)。
...
#### 引入PQL依赖
PQL可以在任何Java和Scala项目里使用，包括Spring Boot项目，只要引入一个依赖即可。[OneApi统一接口](/doc/oneapi/overview)和[Voyager模板引擎](/doc/voyager/overview)也一样引用这个依赖。PQL项目暂时使用阿里云私有仓库保存，未来会迁移到Maven中央仓库。
...
io.qross.pql.PQL类还有很多重载和方法用于应对各种场景，请参阅[PQL类文档](/doc/pql/class)。这种方式特别适合于数据计算脚本的开发，甚至不需要写Java和Scala代码。脚本开发调试完成之后，直接将PQL文件部署到服务器上即可，甚至不需要编译和构建。在服务器上可以通过命令直接运行或者通过[Keeper调度工具](/doc/keeper/overview)运行。作者之前的数据部门采用这种开发方式，非常方便。
...
这种方式就和运行SQL语句的方式一样，不过这种方式采用的不多，毕竟每个PQL过程都是一个完整的处理逻辑，不是某个程序的一部分。
...
```
在本地也可以通过这种方式运行，需要指定文件的绝对路径。可参阅[Worker相关文档](/doc/worker/overview)获取更多信息。

### PQL应用

#### 统一接口OneApi
**OneApi** 是PQL的一种应用，可以通过PQL编辑Web服务的接口，以帮助后端工程师减少大量的重复代码编写工作，快速实现业务逻辑。请参阅[OneApi文档](/doc/oneapi/overview)获取更多信息。

#### 模板引擎Voyager
**Voyager** 是PQL的另一个应用，和OneApi一样，也是应用于Web开发。其同类是FreeMarker或Thymeleaf，不过更简单直接，不需要记各种标记语法，直接在网页HTML代码中写SQL语句进行查询并输出数据。请参阅[Voyager文档](/doc/master/overview)获取更多信息。

#### 数据管理平台Master
**Master** 是一个综合各种数据开发相关功能的管理平台，可以在上面编写PQL并直接运行、部署调度任务、管理服务接口等。请参阅[Master文档](/doc/master/overview)获取更多信息。
...
* [数据管理平台 Master](/doc/master/overview)